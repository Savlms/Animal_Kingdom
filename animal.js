...
class Animal {
    ...
    const may = new Arthropoda("May", "Butterfly");
    const git = new Fish("Git", "Catfish");
    const sip = new Amphibia("Sip", "Forg");
    const plif = new Reptile("Fly", "Totise");
    const nino = new AVES("Nino", "Eagle");
    const nala = new Mammal("Nala", "Cat");
    
    const animals = [may, git, sip, plif, nino, nala];
    animals.forEach(animal => {console.log(animal.allProperties()) + "\n"});
    
    console.log(("OOPS!!! I misspelled Frog\n").toUpperCase());
    
    //USING THE SETTER TO CORRECT THE TYPE PROPERTY
    sip.type = "Frog"
    console.log(sip.allProperties());